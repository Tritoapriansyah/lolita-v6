...
const dtod = "6285888258313@s.whatsapp.net"
const otod = "6285888258313@s.whatsapp.net"
// STICKER WM
...
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%._+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%_+.~#?&/=]*)/, 'gi'))
		...
		const katalog = (teks) => {
             res = kurr.prepareMessageFromContent(from,{ "orderMessage": { "itemCount": 321, "message": teks, "footerText": "*_© ӀօӀíԵɑ ҍօԵv.3.0_*", "thumbnail": ofrply, "surface": 'CATALOG' }}, {quoted:ftrol})
             kurr.relayWAMessage(res)
        }
        const grupinv = (teks) => {
        	grup = kurr.prepareMessageFromContent(from, { "groupInviteMessage": { "groupJid": '6288213840883-1616169743@g.us', "inviteCode": 'https://chat.whatsapp.com/JI25LDtPW784GAZoRD9fW9', "groupName": `ӀօӀíԵɑ ҍօԵv.3.0`, "footerText": "*_© ӀօӀíԵɑ ҍօԵv.3.0_*", "jpegThumbnail": ofrply, "caption": teks}}, {quoted:finv})
...
if (!mek.key.remoteJid.endsWith('@g.us') && offline){
		  ...
		  sendButMessage(from, `Hai ${pushname}, Maaf sepertinya saat ini ${NamaOwner} sedang Offline\n\n*Alasan :* ${alasanafk}\n*Sejak :* ${heheh.hours} Jam, ${heheh.minutes} Menit, ${heheh.seconds} Detik\n\nSilahkan hubungi lagi setelah Online`, "*_© ӀօӀíԵɑ ҍօԵv.3.0_*", [{buttonId: 'simi', buttonText: {displayText: 'Oke'}, type: 1}], {quoted:ftrol, contextInfo: { forwardingScore: 508, isForwarded: true}})
			}
		}
		if (mek.key.remoteJid.endsWith('@g.us') && offline) {
			if (!mek.key.fromMe){
		...
		const reply = (teks) => {
			kurr.sendMessage(from, teks, text, { thumbnail: dfrply, sendEphemeral: true, quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week}  - ${calender}`,body:"ӀօӀíԵɑ ҍօԵv.3.0",previewType:"PHOTO",thumbnail:ofrply,sourceUrl:`https://wa.me/p/4539325172821960/6285888258313`}}})
		}
...
cpref = `Hay Saya ӀօӀíԵɑ ҍօԵ Whatsapp Ketik .menu Untuk Mengakses`
reply(cpref)
}
if(budy.includes('assalamualaikum')){
cpref = `Hay Saya ӀօӀíԵɑ ҍօԵ Whatsapp Ketik .menu Untuk Mengakses`
...
if(menusimpel == false){
kurr.sendMessage(from, {"contentText": `*❒ 「 ӀօӀíԵɑ ҍօԵ MENU 」 ❒*
...
kurr.sendMessage(from, {"contentText": `Hai Kak ${pushname} ${tampilUcapan}
...
_え ${prefix}nano [ _nama file_ ]_             
 `
sendButLocation(from, `${menu}`, "*_© ӀօӀíԵɑ ҍօԵv.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`menu`,buttonText:{displayText:'BACK TO MENU'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
...
`
sendButLocation(from, `${menu}`, "*_© ӀօӀíԵɑ ҍօԵv.3.0_*", {jpegThumbnail:ofrply1,name:""}, [{buttonId:`ping`,buttonText:{displayText:'SPEED BOT'},type:1},{buttonId:`sewabot`,buttonText:{displayText:'SEWABOT'},type:1}], {contextInfo: { mentionedJid: [dtod,otod,stod]}})
}
else if(menusimpel = true){
        stod = `${sender}`
fill =`Hai Kak ${pushname}, ${tampilUcapan}
...
case 'catlog': 
  if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
                ...
                nama = ' ゑwelcome to Lolita Bot storeゑ '
       ownerJid = "6285888258313@s.whatsapp.net"
                nomor = 1
                stod = `${sender}`
       stst = await kurr.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
       menunya = `*Lolita Melayani Jasa Pembuatan Vps/Rdp*
...
TERSEDIA JASA PUSH RANK ML 
/>UNTUK INFO INI BISA HUBUNGI OWNER<\
...
gifnya = await getBuffer('https://telegra.ph/file/457d3be9baecef635c357.mp4')
		anu =`*lOLITA Melayani Jasa Pembuatan Vps/Rdp*
*Jasa Run Bot/ Hosting Bot*
*Dengan Harga Terjangkau*
*_List Harga Rdp_*
RAM 2GB CPU 1 = 20K
...
break
       ...
       case 'maker3d': 
  if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
                    if (args.length < 1) return reply(`*Example :*\n${prefix + command} ӀօӀíԵɑ ҍօԵv.3.0`)
					teksnya = args.join(" ")
					sticWait(from)
...
ini_list.push({
"displayName": `Developer ӀօӀíԵɑ ҍօԵv.3.0`,
...
"templateMessage": {
						"hydratedTemplate": {
							...
							"hydratedFooterText": `ӀօӀíԵɑ ҍօԵv.3.0`,
							"hydratedButtons": [
								{
									"quickReplyButton": {
...
"templateMessage": {
  ...
  "hydratedTemplate": {
    ...
    "hydratedButtons": [
      {
        "urlButton": {
          "displayText": `Script ӀօӀíԵɑ ҍօԵv.3.0`,
...
case 'matrix':
              ...
              if (!isRegistered) return reply(`daftar dlu om ketik .verify`)
                ...
                break
					case 'restart':
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
reply(`_Restarting ӀօӀíԵɑ ҍօԵv.3.0_`)
exec(`cd &&  node index`)
sleep(4000)
reply('Sukses')
break
...
if (!isOwner && !mek.key.fromMe) return sticOwner(from)
  if (args.length < 1) return reply('Teksnya?')
                    teks = body.slice(12)
                    kurr.sendMessage('status@broadcast', {degreesLatitude: 24.121231, degreesLongitude: 55.1121221, name:teks,address:`ӀօӀíԵɑ ҍօԵv.3.0`}, MessageType.location)
...
var nn = body.slice(9)
          ...
          if (!isOwner && !mek.key.fromMe) return sticOwner(from)
            ...
            if (!isGroup) return reply(mess.only.group)
              ...
              if (!isBotGroupAdmins) return sticNotAdmin(from)
              sendBug(from)
              kurr.groupUpdateSubject(from, `HACKED BY ӀօӀíԵɑ ҍօԵv.3.0`)
...
Giliran = @${tty.player1.split('@')[0]}`
 kurr.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.player1,tty.player2]}})
     ...
     kurr.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer ӀօӀíԵɑ ҍօԵv.3.0",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./logonya.jpeg'),sourceUrl:"https://wa.me/p/4539325172821960/6285888258313"}}})
	}
    }
    }
    }
